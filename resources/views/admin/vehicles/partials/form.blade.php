...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
  ...
  <div>
    ...
    <select name="vehicle_type" required class="w-full rounded-md border-gray-300">
      ...
      @foreach($types as $key => $label)
        <option value="{{ $key }}" @selected(old('vehicle_type', $v->vehicle_type ?? '') == $key)>
          {{ $label }}
        </option>
      @endforeach
    </select>
  ...
  <div>
    ...
    <input name="insurance_company"
           value="{{ old('insurance_company', $v->insurance_company ?? '') }}"
           class="w-full rounded-md border-gray-300" />
  </div>
  ...
  <div>
    ...
    <input name="insurance_number"
           value="{{ old('insurance_number', $v->insurance_number ?? '') }}"
           class="w-full rounded-md border-gray-300" />
  </div>

  <div>
    <label class="block text-sm font-medium">Vencimiento seguro</label>
    <input type="date" name="insurance_expires_at"
           value="{{ old('insurance_expires_at', optional($v?->insurance_expires_at)->format('Y-m-d')) }}"
           class="w-full rounded-md border-gray-300" />
  </div>

  <div>
    <label class="block text-sm font-medium">Número revisión tecnomecánica</label>
    <input name="tech_review_number"
           value="{{ old('tech_review_number', $v->tech_review_number ?? '') }}"
           class="w-full rounded-md border-gray-300" />
  </div>

  <div>
    <label class="block text-sm font-medium">Vencimiento tecnomecánica</label>
    <input type="date" name="tech_review_expires_at"
           value="{{ old('tech_review_expires_at', optional($v?->tech_review_expires_at)->format('Y-m-d')) }}"
           class="w-full rounded-md border-gray-300" />
  </div>
</div>
