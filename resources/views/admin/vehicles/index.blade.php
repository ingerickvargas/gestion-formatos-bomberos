<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.vehicles.create') }}" class="px-4 py-2 bg-red-600 text-white rounded-md">Nuevo</a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white shadow rounded p-4">
                <form class="flex gap-2 flex-wrap">
                    <input name="search" value="{{ request('search') }}" placeholder="Buscar por placa, marca, seguro..." class="flex-1 min-w-[240px] rounded-md border-gray-300" />
            ...
            <div class="bg-white shadow rounded overflow-hidden">
				<div class="p-6 border-b">
                    <p class="text-sm text-gray-600">
                        Total (página): {{ $vehicles->count() }} — Mostrando {{ $vehicles->firstItem() }} a {{ $vehicles->lastItem() }} de {{ $vehicles->total() }}
                    </p>
                </div>
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        @forelse($vehicles as $v)
                            <tr class="border-t">
                                ...
                                <td class="p-3 text-right">
                                    ...
                                    <form class="inline" method="POST" action="{{ route('admin.vehicles.destroy', $v) }}" onsubmit="return confirm('¿Eliminar vehículo?')">
                                        @csrf @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td class="p-3" colspan="7">No hay vehículos registrados.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
