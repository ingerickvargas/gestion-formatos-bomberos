<x-app-layout>
  <x-slot name="header">
    <div class="flex items-center justify-between">
      ...
      <a href="{{ route('formats.vehicle-cleanings.create') }}"
         class="px-4 py-2 bg-red-600 text-white rounded-md">Nuevo</a>
    </div>
  </x-slot>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          <div>
            ...
            <input name="plate" value="{{ request('plate') }}"
              class="w-full rounded-md border-gray-300" placeholder="Buscar por placa...">
          </div>
                ...
                <tr>
                  ...
                  <td class="px-3 py-2">
                    <span class="px-2 py-1 rounded text-xs font-semibold
                      {{ $c->cleaning_type==='TERMINAL' ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'bg-green-50 text-green-700 border border-green-200' }}">
                      {{ $c->cleaning_type }}
                    </span>
                  </td>
