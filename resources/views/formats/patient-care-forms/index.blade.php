    ...
    <div class="bg-white shadow rounded p-4">
	  <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
		...
		<div>
		  ...
		  <input name="search" value="{{ request('search') }}" placeholder="Paciente o documento"
				 class="w-full rounded-md border-gray-300">
		</div>
		<div>
		  <label class="block text-sm font-medium">Mes</label>
		  <input type="month" name="filled_month"
    ...
    <div class="bg-white shadow rounded">
      <table class="min-w-full text-sm">
        ...
        <tbody class="divide-y">
            ...
            <tr>
			  ...
			  <td class="px-3 py-2">
				@if($f->attachment_path)
				  <a href="{{ asset('storage/'.$f->attachment_path) }}" target="_blank"
					 class="inline-flex items-center px-2 py-1 text-xs rounded bg-green-100 text-green-800">
					 Foto adjunta
				  </a>
				@else
				  <span class="inline-flex items-center px-2 py-1 text-xs rounded bg-gray-100 text-gray-600">
					 No adjunta
				  </span>
				@endif
			  </td>
          ...
          @empty
            <tr><td class="px-3 py-6 text-center text-gray-500" colspan="6">Sin registros</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
