<x-app-layout>
	<div class="space-y-6">
	  ...
	  <form
		method="POST"
		action="{{ route('formats.patient-care-forms.update', $form) }}"
	  ...
	  >
		...
		<div class="bg-white shadow rounded p-4 space-y-3">
		  <div class="flex items-center justify-between">
			...
			@if($form->attachment_path)
			  <form method="POST" action="{{ route('formats.patient-care-forms.attachment.delete', $form) }}">
				@csrf
				@method('DELETE')
				<button class="px-3 py-2 border rounded-md bg-red-600 text-white" type="submit">
				  Quitar imagen
				</button>
			  </form>
			@endif
		  ...
		  @if($form->attachment_path)
			<div class="text-sm">
			  <a class="underline" href="{{ route('formats.patient-care-forms.attachment', $form) }}">
				Descargar imagen actual
			  </a>
			</div>
			<div class="mt-2">
			  <img
				src="{{ Storage::url($form->attachment_path) }}"
				alt="Adjunto"
				style="max-height: 240px; border:1px solid #ddd; border-radius:8px;"
			  >
			</div>
		  @else
		  ...
		  <div>
			...
			@error('attachment') <div class="text-sm text-red-600 mt-1">{{ $message }}</div> @enderror
		  </div>
		</div>
		@include('formats.patient-care-forms.partials.form', ['form' => $form])
		<div class="flex justify-end gap-2">
		  <a href="{{ route('formats.patient-care-forms.index', $form) }}" class="px-4 py-2 border rounded-md bg-sky-800 text-white">
			Cancelar
		  </a>
