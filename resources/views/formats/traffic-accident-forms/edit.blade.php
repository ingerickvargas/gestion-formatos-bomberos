<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar accidente de tránsito #{{ $form->id }}</h2>
    ...
    <div class="space-y-4">
        ...
        <form method="POST" action="{{ route('formats.traffic-accident-forms.update', $form) }}" class="space-y-6">
            ...
            <div class="flex items-center justify-end gap-2">
                <a href="{{ route('formats.traffic-accident-forms.index', $form) }}" class="px-4 py-2 border rounded-md bg-sky-800 text-white">Cancelar</a>
                <button type="submit" class="px-5 py-2 rounded-md bg-red-600 text-white">Actualizar</button>
            </div>
        </form>
    </div>
</x-app-layout>
