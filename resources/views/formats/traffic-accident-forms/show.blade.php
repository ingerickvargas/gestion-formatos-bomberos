        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                    Accidente de tránsito #{{ $form->id }}
                </h2>
                <p class="text-sm text-gray-500">
                    Fecha atención:
                    <span class="font-medium">
                        {{ optional($form->attention_date)->format('Y-m-d') ?? '-' }}
                    </span>
                    — Hora: <span class="font-medium">{{ $form->attention_time ?? '-' }}</span>
                </p>
            </div>

            <div class="flex items-center gap-2">
                <a href="{{ route('formats.traffic-accident-forms.edit', $form) }}"
				...
				<a href="{{ route('formats.traffic-accident-forms.export-pdf', $form) }}"
				   target="_blank"
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white shadow rounded p-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                ...
                <div class="md:col-span-2">
                    <span class="text-gray-500">Quien informa:</span>
                    <span class="font-medium">{{ $form->informer->name ?? '-' }}</span>
                </div>
            ...
            <div class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div class="md:col-span-2">
                    <span class="text-gray-500">Paciente / Víctima:</span>
                    <span class="font-medium">{{ $form->patient_name ?? '-' }}</span>
                </div>
                <div>
                    <span class="text-gray-500">Tipo doc:</span>
                    <span class="font-medium">{{ $form->patient_doc_type ?? '-' }}</span>
                    <span class="text-gray-500"> / No:</span>
                    <span class="font-medium">{{ $form->patient_doc_number ?? '-' }}</span>
                </div>

                <div>
                    <span class="text-gray-500">Nacimiento:</span>
                    <span class="font-medium">{{ optional($form->patient_birth_date)->format('Y-m-d') ?? '-' }}</span>
                </div>
                ...
                <div><span class="text-gray-500">Tel acomp:</span> <span class="font-medium">{{ $form->companion_phone ?? '-' }}</span></div>
            </div>
        </div>
        ...
        <div class="bg-white shadow rounded p-4">
            ...
            <div class="grid grid-cols-2 md:grid-cols-6 gap-4 text-sm">
                ...
                <div>
                    <span class="text-gray-500">RO/RV/RM:</span>
                    <span class="font-medium">{{ $form->ro ?? '-' }}</span> /
                    <span class="font-medium">{{ $form->rv ?? '-' }}</span> /
                    <span class="font-medium">{{ $form->rm ?? '-' }}</span>
                </div>
            </div>
            ...
            <div class="mt-4 text-sm">
                ...
                <div class="font-medium">
                    {{ count($injuries) ? implode(', ', $injuries) : '-' }}
                </div>
            </div>
        ...
        <div class="bg-white shadow rounded p-4">
            ...
            <div class="text-sm font-medium">
                {{ count($procedures) ? implode(', ', $procedures) : '-' }}
            </div>
        </div>
        ...
        <div class="bg-white shadow rounded p-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                    <span class="text-gray-500">Vehículo institución:</span>
                    <span class="font-medium">{{ $form->vehicle->plate ?? '-' }}</span>
                </div>
                <div class="md:col-span-2">
                    <span class="text-gray-500">Responsable:</span>
                    <span class="font-medium">
                        {{ $form->responsibleUser->name ?? '-' }}
                        @if($form->responsible_document)
                            ({{ $form->responsible_document }})
                        @endif
                    </span>
                </div>
            </div>
