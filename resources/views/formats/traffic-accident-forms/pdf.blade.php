...
<div class="subtitle">
  Registro #{{ $form->id }} —
  Diligenciado por: {{ $form->creator->name ?? '—' }}
</div>
...
<div class="box">
  <table class="grid2">
    ...
    <tr>
      <td><b>Quién informa:</b> {{ $form->informerUser->name ?? '-' }}</td>
      <td><b>Historia clínica:</b> {{ $form->clinical_history ?? '-' }}</td>
    </tr>
    <tr>
      <td><b>Fecha atención:</b> {{ optional($form->attention_date)->format('Y-m-d') ?? '-' }}</td>
      <td>
        <b>Hora salida:</b> {{ $form->departure_time ?? '-' }} |
        <b>Hora atención:</b> {{ $form->attention_time ?? '-' }}
      </td>
    </tr>
  </table>
</div>
