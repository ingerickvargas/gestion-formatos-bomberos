<x-app-layout>
    <div class="max-w-6xl mx-auto space-y-6">
        ...
        @endphp
        <form method="POST" action="{{ route('formats.vehicle-exit-reports.store') }}" x-data="{ users: @js($usersJson), selectedDriverId: '{{ old('driver_user_id', '') }}', driver: { document: '', phone: '' }, updateDriver() { const u = this.users.find(x => String(x.id) === String(this.selectedDriverId)); this.driver.document = u ? (u.document ?? '') : ''; this.driver.phone = u ? (u.phone ?? '') : ''; } }" x-init="updateDriver()" class="space-y-6">
            @csrf
            @if ($errors->any())
            ...
            <div class="bg-gray-50 rounded p-4 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <select name="driver_user_id" x-model="selectedDriverId" @change="updateDriver()" required class="w-full rounded-md border-gray-300">
                    ...
                    <div>
                        ...
                        <input type="text" x-model="driver.document" readonly class="w-full rounded-md border-gray-300 bg-gray-100">
                    </div>
                    <div>
                        <label class="block text-sm font-medium">Teléfono</label>
                        <input type="text" x-model="driver.phone" readonly class="w-full rounded-md border-gray-300 bg-gray-100">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white shadow rounded p-4 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input name="department" required value="{{ old('department') }}" class="w-full rounded-md border-gray-300" placeholder="Departamento">
                    ...
                    <div>
                        ...
                        <input name="city" required value="{{ old('city') }}" class="w-full rounded-md border-gray-300" placeholder="Ciudad / Municipio">
                        @error('city') <div class="text-sm text-red-600 mt-1">{{ $message }}</div> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium">Barrio</label>
                        <input name="neighborhood" value="{{ old('neighborhood') }}" class="w-full rounded-md border-gray-300" placeholder="Barrio">
                    ...
                    <div>
                        ...
                        <input name="vereda" value="{{ old('vereda') }}" class="w-full rounded-md border-gray-300" placeholder="Vereda">
                    ...
                    <div>
                        ...
                        <input name="nomenclature" value="{{ old('nomenclature') }}" class="w-full rounded-md border-gray-300" placeholder="Nomenclatura">
                    ...
                    <div>
                        ...
                        <input type="time" name="departure_time" required value="{{ old('departure_time') }}" class="w-full rounded-md border-gray-300">
                        @error('departure_time') <div class="text-sm text-red-600 mt-1">{{ $message }}</div> @enderror
                    </div>
                </div>
            </div>
            <div class="flex justify-end gap-2">
                <a href="{{ route('formats.vehicle-exit-reports.index') }}" class="px-4 py-2 border rounded-md bg-sky-800 text-white">Cancelar</a>
                <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-gray-800">Guardar y asignar</button>
            </div>
        </form>
    </div>
</x-app-layout>
