<x-app-layout>
    <div class="space-y-4">
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
                ...
                @if($report->status === 'PENDING_DRIVER' && $report->driver_user_id === auth()->id())
                    <a href="{{ route('formats.vehicle-exit-reports.driver-form', $report) }}" class="px-3 py-2 bg-gray-900 text-white rounded-md">Diligenciar</a>
                @endif
            </div>
        </div>
        @if(session('success'))
        ...
        <div class="bg-white shadow rounded p-4 grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
            <div><span class="text-gray-500">Estado:</span> <b>{{ $report->status }}</b></div>
            <div><span class="text-gray-500">Creado:</span> <b>{{ $report->created_at->format('Y-m-d H:i') }}</b></div>
            <div><span class="text-gray-500">Guardia:</span> <b>{{ $report->guardUser->name ?? '-' }}</b></div>
            <div><span class="text-gray-500">Conductor:</span> <b>{{ $report->driverUser->name ?? '-' }}</b></div>
        </div>
        ...
        <div class="bg-white shadow rounded p-4 space-y-2 text-sm">
            ...
            @else
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div><span class="text-gray-500">Diligenciado:</span> <b>{{ ($report->updated_at)->format('Y-m-d H:i') ?? '-' }}</b></div>
                </div>
                <div class="mt-3">
