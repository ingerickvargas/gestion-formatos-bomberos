...
<div x-data="{
...
}" x-init="initItems(); loadVehicle()" class="space-y-4">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
            ...
            <input type="date" name="inventory_date" required value="{{ $initialDate }}" class="w-full rounded-md border-gray-300">
        ...
        <table class="min-w-full text-sm">
            ...
            <tbody class="divide-y">
                @foreach($supplies as $s)
                    <tr>
                        <td class="px-3 py-2">
                            <input type="checkbox" class="rounded border-gray-300" x-model="itemsById[{{ $s->id }}].checked" @change="if(!itemsById[{{ $s->id }}].checked){ itemsById[{{ $s->id }}].qty=''; }" name="items[{{ $s->id }}][checked]" value="1" />
                        </td>
                        <td class="px-3 py-2">
                            <span class="text-gray-900">{{ $s->name }}</span>
                        </td>
                        <td class="px-3 py-2">
                            <input type="number" min="1" step="1" class="w-full rounded-md border-gray-300 focus:border-gray-500 focus:ring-gray-500 disabled:bg-gray-100" :disabled="!itemsById[{{ $s->id }}].checked" :required="itemsById[{{ $s->id }}].checked" x-model="itemsById[{{ $s->id }}].qty" name="items[{{ $s->id }}][quantity]" placeholder="0" />
                        </td>
                    </tr>
                @endforeach
            </tbody>
